...
class FrameAttentivePooling(nn.Module):

    def __init__(self, hidden_states_dim: int, pooling_type='sum'):
        assert pooling_type in ['sum', 'mean']
        super(FrameAttentivePooling, self).__init__()
        self.linear = nn.Linear(hidden_states_dim, 1)
    ...
    def forward(self, x, attention_mask=None):
        ...
        attention_mask:(mask[B,t_dim],last_feat_pos[4])
        return:size (B, H)
        """

        attention_score = self.linear(x).squeeze(-1)

        if attention_mask is not None:
            attention_score = attention_score + attention_mask[0]
        norm_attention_score = F.softmax(attention_score, dim=1).unsqueeze(-1)
        if self.pooling_type == "sum":
            return torch.sum(norm_attention_score * x, dim=1)
        if self.pooling_type == "mean":
            return torch.mean(norm_attention_score * x, dim=1)
...
if __name__ == '__main__':
    ...
    # pool = SelfAttentionPooling(input_dim=768)
    pool = FrameAttentivePooling(hidden_states_dim=768, pooling_type='sum')
    # attention_masks = torch.ones((4, 399))
    print(pool(x).shape)
