...
# test the loss :AngularAdditiveMarginSoftMaxLoss
...
class Wav2Vec2EmoNet(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        """
        self.classifier = AngularAdditiveMarginSoftMaxLoss(input_features=ARGS.HIDDEN_STATES_DIM,
                                                           output_features=num_classes)

        # initialize model parameters
        self.reset_parameters()

    def forward(self, x, length, label=None):
        with torch.no_grad():
        ...
        loss, pred_ = self.classifier(logits, label)
        return loss, pred_
